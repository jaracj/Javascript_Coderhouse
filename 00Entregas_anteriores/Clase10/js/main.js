...
function mostrarFeedback(texto, tipo) {
    Toastify({
        ...
        position: "right",
        style: {
            background: tipo === "exito" ? "linear-gradient(to right, #00b09b, #96c93d)" :
                       tipo === "error" ? "linear-gradient(to right, #ff416c, #ff4b2b)" :
...
function eliminarLibro(id) {
    ...
    const librosActualizados = libros.filter(l => l.id !== id);
    ...
    try {
        ...
        mostrarFeedback("No se pudo procesar la operacion, intenta de nuevo.", "error");
    } finally {
        renderizarLibros();
    }
}
...
async function cargarLibros() {
    contenedorLibros.innerHTML = "<p class='cargando'>Cargando libros...</p>";
    ...
    try {
        ...
        if (librosGuardados) {
            ...
            libros = datos.map(d => new Libro(d.id, d.titulo, d.autor, d.genero, d.paginas));
            localStorage.setItem("libros", JSON.stringify(libros));
        }
        
        mostrarFeedback("Libros cargados con exito", "exito");
        renderizarLibros();
        
    } catch (error) {
        mostrarFeedback("Error al cargar los libros. Intenta de nuevo.", "error");
